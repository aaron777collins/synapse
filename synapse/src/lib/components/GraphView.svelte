<script lang="ts">
  ...
  async function initGraph() {
    ...
    nodes = data.nodes.map((n) => ({
      id: n.id,
      // The server returns { id } without a title field; derive a readable
      // label by stripping the directory path and .md extension.
      title: n.title || n.id.replace(/\.md$/i, "").split("/").pop() || n.id,
...
{#if $graphOpen}
  ...
  >
    <!-- Header bar with title and close — gives the overlay a proper chrome -->
    <div
      class="flex items-center justify-between px-5 py-3 shrink-0"
      style="background: var(--surface); border-bottom: 1px solid var(--border);"
    >
      <span class="font-semibold text-sm tracking-wide" style="color: var(--text);">
        Graph View
      </span>
      <!-- More refined close: rounded-xl and slightly larger hit area -->
      <button
        onclick={() => graphOpen.set(false)}
        class="p-2.5 rounded-xl transition-colors hover:bg-[var(--surface-hover)]"
    ...
    <!-- Canvas fills the remaining space; dot-grid pattern gives spatial context -->
    <canvas
      bind:this={canvas}
      class="flex-1 w-full graph-canvas"
      onwheel={handleWheel}
...
<style>
  /* Subtle dot-grid makes the infinite canvas feel grounded */
  .graph-canvas {
